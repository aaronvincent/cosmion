!note the fehlberg step is the same, so that stayed in the other file (rkf45full.f90)
!this variant returns a vector of time and an array of y at those times
subroutine rkf45fullhistory ( f, neqn, y, yp, t, tout, relerr, abserr, flag,tvec,yarr )
...
!!Extra parameters for returning full solution
! Size of this array can't be larger than the maximum number of steps
  real ( kind = rk ), intent(out) :: tvec(maxnfe)
  real ( kind = rk ), intent(out) :: yarr(maxnfe,neqn)
...
!
      ...
      call fehlfull ( f, neqn, y, t, h, yp, f1, f2, f3, f4, f5, f1 )
      nfe = nfe + 5
